...
function isOrphanFenceChunk(text) {
 ...
 * @returns {{ chunks: Array<{type:'prose'|'code', text?: string, lang?: string}>, openCode: {lang:string,text:string}|null }}
 */
function splitMarkdownFences(content, streaming = false) {
  ...
  // Legacy open-fence scan for streaming when fence header line not closed
  if (streaming) {
    openFenceLen = 0;
    openLang = '';
    openFenceLine = -1;
